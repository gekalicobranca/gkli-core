...
export default async function ReguaSmtpPage({ searchParams }: { searchParams?: Promise<Params> }) {
  ...
  return (
    <ReguaShell usuario={context.usuario}>
      ...
      <section className="suite-panel regua-smtp-selector">
        <form method="get" className="regua-form regua-smtp-scope-form">
          <label><span>Conta de envio</span><select name="carteira" defaultValue={carteiraId}><option value="">Fallback global do app</option>{(carteiras ?? []).map((item: any) => <option key={item.id} value={item.id}>{item.codigo} · {item.nome}</option>)}</select></label>
      ...
      <section className="regua-smtp-status-grid">
        <article className="metric-card"><span className="metric-label">Status</span><strong className="metric-value">{label}</strong><span className="metric-hint">{status.configured ? 'Servidor e remetente informados' : 'Preencha a configuração'}</span></article>
        <article className="metric-card"><span className="metric-label">Origem</span><strong className="metric-value">{status.source === 'environment' ? 'Ambiente' : status.source === 'carteira' ? 'Carteira' : status.source === 'global' ? 'Banco de dados' : 'Não configurado'}</strong><span className="metric-hint">configuração ativa para o escopo</span></article>
        <article className="metric-card"><span className="metric-label">Servidor</span><strong className="metric-value regua-smtp-host">{status.host || '—'}</strong><span className="metric-hint">{status.port} · {status.secure ? 'SSL direto' : status.starttls ? 'STARTTLS' : 'sem TLS'}</span></article>
      </section>
      ...
      <section className="regua-smtp-layout">
        <form action={salvarSmtpRegua} className="suite-panel regua-smtp-config-form">
          ...
          <div className="suite-panel-heading"><div><p className="suite-section-kicker">SMTP</p><h2>Configuração de envio</h2><p>A senha é criptografada no servidor e nunca é exibida depois de salva.</p></div><button className="button">Salvar SMTP</button></div>
        ...
        <form action={testarSmtpRegua} className="suite-panel regua-smtp-test-form">
          ...
          <button className="button" disabled={!status.configured || !status.active}>Enviar teste</button>
