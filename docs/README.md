# Escopo do projeto

O `gkli-core` é a base administrativa da Gekali e hospeda o GKLI Régua.

## GKLI Core

- Login e sessão com Supabase Auth.
- Administração de usuários, tipos, carteiras e times.
- Perfis, permissões e acessos por sistema.
- Catálogo de sistemas e auditoria administrativa.

## GKLI Régua

- Carteiras e clientes próprios.
- Importação de bases XLSX, XLS e CSV.
- Templates de e-mail.
- Preparação de lotes para revisão e envio.
- Configuração SMTP global ou por carteira.
- Teste de envio com credenciais protegidas no servidor.

Outros produtos GKLI e qualquer produto GKIT não fazem parte deste repositório.
