...
function emails(value: unknown) {
  return [...new Set(String(value ?? '').replace(/^"|"$/g, '').split(/[;,]/).map((item) => item.trim().toLowerCase()).filter(validEmail))]
}
async function workbookFromFile(arquivo: File) {
  const buffer = await arquivo.arrayBuffer()
  if (arquivo.name.toLowerCase().endsWith('.csv')) {
    return XLSX.read(new TextDecoder('windows-1252').decode(buffer), { type: 'string', cellDates: true, FS: ';' })
  }
...
export async function importarCredoresRegua(formData: FormData) {
  ...
  if (!(arquivo instanceof File) || !arquivo.size) throw new Error('Selecione o arquivo REL_CREDOR.csv.')
  const workbook = await workbookFromFile(arquivo)
  const sheet = workbook.Sheets[workbook.SheetNames[0]]
  const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '' })
  const carteiras = rows.map((raw) => {
    const row = Object.fromEntries(Object.entries(raw).map(([key, value]) => [normalize(key.replace(/^\uFEFF/, '')), value]))
    ...
    return {
      ...
      descricao: 'Carteira importada da referência de credores.',
      metadata: {
  ...
  if (!carteiras.length) throw new Error('Nenhum credor válido foi encontrado no arquivo de referência.')
  const { error } = await admin().schema('gkli_regua').from('carteiras').upsert(carteiras, { onConflict: 'codigo' })
  if (error) throw new Error(error.message)
  revalidatePath('/modulos/gkli-regua')
}
...
export async function importarBaseRegua(formData: FormData) {
  ...
  const { data: carteiras, error: carteiraError } = await db.from('carteiras').select('id,codigo').eq('status', 'ativo')
  if (carteiraError) throw new Error(carteiraError.message)
  const carteiraPorCodigo = new Map((carteiras ?? []).map((item: any) => [String(item.codigo).trim(), item.id]))
  const { data: importacao, error: importError } = await db.from('importacoes').insert({
    carteira_id: null, arquivo_nome: arquivo.name, status: 'processando', total_linhas: rows.length, criado_por: context.usuario.id,
  }).select('id').single()
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const carteiraId = carteiraPorCodigo.get(codigoCredor) ?? null
    const nome = String(row.nome ?? row.cliente ?? row.devedor ?? row.nome_devedor ?? '').trim()
    ...
    const motivo = !carteiraId ? `Credor ${codigoCredor || 'não informado'} não encontrado na referência` : !nome ? 'Nome não informado' : !destinatarios.length ? 'E-mail ausente ou inválido' : null
    const status = motivo ? 'invalido' : 'apto'
    if (motivo) invalidas += 1; else validas += 1
    itens.push({
      importacao_id: importacao.id, carteira_id: carteiraId, linha: index + 2, nome: nome || `Linha ${index + 2}`, email: email || null, documento,
      valor: money(row.valor ?? row.valor_devido ?? row.saldo ?? row.vl_titulo ?? row.vl_saldo), vencimento: isoDate(row.vencimento ?? row.data_vencimento ?? row.dt_vencimento),
      referencia: String(row.referencia ?? row.contrato ?? row.unidade ?? row.cod_devedor ?? row.cod_titulo ?? '').trim() || null,
      dados: { ...raw, _emails: destinatarios, _codigo_credor: codigoCredor }, status, motivo,
    })
  }
...
export async function gerarLoteRegua(formData: FormData) {
  ...
  const carteiraId = text(formData, 'carteira_id')
  const nome = text(formData, 'nome')
  if (!importacaoId || !templateId || !carteiraId || !nome) throw new Error('Informe carteira, importação, template e nome do lote.')
  const db = admin().schema('gkli_regua')
  const [{ data: importacao, error: importError }, { data: template, error: templateError }, { data: itens, error: itemError }] = await Promise.all([
    ...
    db.from('importacao_itens').select('*').eq('importacao_id', importacaoId).eq('carteira_id', carteiraId).eq('status', 'apto'),
  ])
  ...
  const { data: lote, error: loteError } = await db.from('lotes').insert({
    carteira_id: carteiraId, importacao_id: importacaoId, template_id: templateId, nome,
  ...
  const loteItens = itens.flatMap((item: any) => {
    ...
    const destinatarios = Array.isArray(item.dados?._emails) && item.dados._emails.length ? item.dados._emails : [item.email]
    return destinatarios.map((destinatario: string) => ({
      lote_id: lote.id, importacao_item_id: item.id, cliente_id: item.cliente_id, destinatario,
  ...
  await db.from('lotes').update({ total_itens: loteItens.length }).eq('id', lote.id)
