...
import type { ReguaCarteira, ReguaData, ReguaImportacao, ReguaLote, ReguaRegistro, ReguaTemplate } from './types'
...
export async function getReguaData(): Promise<ReguaData> {
  ...
  const [carteirasResult, registrosResult, templatesResult, importacoesResult, lotesResult] = await Promise.all([
    db.from('carteiras').select('id,codigo,nome,cnpj,email,descricao,status').order('nome'),
    db.from('importacao_itens').select('id,importacao_id,carteira_id,linha,nome,documento,email,valor,vencimento,referencia,status,motivo,created_at').order('created_at', { ascending: false }).limit(500),
    db.from('templates').select('id,nome,assunto,corpo_html,corpo_texto,status').order('nome'),
    db.from('importacoes').select('id,codigo_lote,carteira_id,arquivo_nome,status,total_linhas,linhas_validas,linhas_invalidas,created_at').order('created_at', { ascending: false }).limit(30),
  ...
  const firstError = [carteirasResult, registrosResult, templatesResult, importacoesResult, lotesResult].find((result) => result.error)?.error
  if (firstError) {
    return {
      databaseReady: false,
      errorMessage: 'A estrutura independente do GKLI Régua ainda não foi aplicada ao banco.',
      carteiras: [], registros: [], templates: [], importacoes: [], lotes: [],
      resumo: { carteiras: 0, registros: 0, templates: 0, importados: 0, preparados: 0 },
    }
  }

  const carteiras = (carteirasResult.data ?? []) as ReguaCarteira[]
  const registros = (registrosResult.data ?? []) as ReguaRegistro[]
