...
export async function requireGkitFlexApiAccess() {
  ...
  if (!appIds.length) {
    return NextResponse.json({ error: 'Modulo GKLI Flex nao cadastrado no Core.' }, { status: 403 })
  }
  ...
  return accessRows?.length ? null : NextResponse.json({ error: 'Sem acesso ao modulo GKLI Flex.' }, { status: 403 })
}
