...
async function requireWrite() {
  const context = await requireModuleAccess('gkli-flex', '/modulos/gkli-flex/colaboradores');
  if (!canAccess(context.permissions, 'gkit_flex.colaboradores.write')) {
    throw new Error('Voce nao tem permissao para gerenciar colaboradores do GKLI Flex.');
  }
  return context;
}
...
export async function createGkitFlexColaboradorAction(formData: FormData) {
  ...
  revalidatePath('/modulos/gkli-flex');
  revalidatePath('/modulos/gkli-flex/colaboradores');
  redirect('/modulos/gkli-flex/colaboradores');
}
...
export async function updateGkitFlexColaboradorAction(formData: FormData) {
  ...
  revalidatePath('/modulos/gkli-flex');
  revalidatePath('/modulos/gkli-flex/colaboradores');
  revalidatePath(`/modulos/gkli-flex/colaboradores/${id}`);
  redirect('/modulos/gkli-flex/colaboradores');
}
