...
async function requireGkitAteWrite(permission = 'gkit_ate.importacoes.write') {
  const context = await requireModuleAccess('gkli-ate')
  if (!canAccess(context.permissions, permission)) {
    throw new Error('Usuario sem permissao para gravar no GKLI ATE.')
  }
  return context
}
...
function revalidateGkitAteCockpit() {
  revalidatePath('/modulos/gkli-ate')
  revalidatePath('/modulos/gkli-ate/atendimentos')
  revalidatePath('/modulos/gkli-ate/cadastros')
  revalidatePath('/modulos/gkli-ate/tarefas')
}
...
export async function importarGkitAteAstreaXlsx(formData: FormData): Promise<ImportGkitAteResult> {
  ...
  revalidatePath('/modulos/gkli-ate')
  revalidatePath('/modulos/gkli-ate/atendimentos')
  revalidatePath('/modulos/gkli-ate/importacoes')
  return result
}
...
export async function createGkitAteAtendimentoAction(formData: FormData) {
  ...
  revalidatePath(`/modulos/gkli-ate/atendimentos/${atendimentoId}`)
  redirect(`/modulos/gkli-ate/atendimentos/${atendimentoId}`)
}
...
export async function createGkitAteTarefaAction(formData: FormData) {
  ...
  if (text(formData, 'return_to') === 'cockpit') {
    redirect('/modulos/gkli-ate?painel=tarefa')
  }
}
...
export async function createGkitAteAtendimentoTipoAction(formData: FormData) {
  ...
  revalidateGkitAteCockpit()
  redirect('/modulos/gkli-ate?painel=tipo-atendimento')
}
...
export async function createGkitAteTarefaTipoAction(formData: FormData) {
  ...
  revalidateGkitAteCockpit()
  redirect('/modulos/gkli-ate?painel=tipo-tarefa')
}
...
export async function completeGkitAteTarefaAction(formData: FormData) {
  ...
  revalidatePath('/modulos/gkli-ate/tarefas')
  revalidatePath(`/modulos/gkli-ate/tarefas/${id}`)
  if (data?.atendimento_id) revalidatePath(`/modulos/gkli-ate/atendimentos/${data.atendimento_id}`)
  redirect(`/modulos/gkli-ate/atendimentos/${data?.atendimento_id ?? atendimentoId}`)
}
