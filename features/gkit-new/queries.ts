...
function schemaHealth(error: any): GkitNewHealth | null {
  ...
  return {
    ok: false,
    title: 'Falha ao ler dados do GKLI New',
...
function setupRow(health: GkitNewHealth): GkitNewListRow {
  return {
    ...
    subtitle: health.message ?? 'Existe uma pendência de configuração do GKLI New.',
    status: 'atenção',
    value: 'Supabase',
...
export async function requireGkitNewContext(target?: string) {
  const context = await requireModuleAccess('gkli-new', target)
  const hasAccess =
...
export function clienteRows(clientes: GkitNewCliente[]): GkitNewListRow[] {
  return clientes.map((cliente) => ({
    ...
    detailHref: `/modulos/gkli-new/clientes/${cliente.id}`,
...
export function contatoRows(contatos: GkitNewContato[]): GkitNewListRow[] {
  return contatos.map((contato) => ({
    ...
    detailHref: `/modulos/gkli-new/contatos/${contato.id}`,
    tone: 'primary',
  }))
}
...
export function workflowRows(modelos: GkitNewWorkflowModelo[]): GkitNewListRow[] {
  return modelos.map((modelo) => ({
    ...
    detailHref: `/modulos/gkli-new/base/workflow/${modelo.id}`,
...
export function oportunidadeRows(oportunidades: GkitNewOportunidade[]): GkitNewListRow[] {
  return oportunidades.map((oportunidade) => ({
    ...
    detailHref: `/modulos/gkli-new/oportunidades/${oportunidade.id}`,
...
export function tarefaRows(tarefas: GkitNewTarefa[]): GkitNewListRow[] {
  return tarefas.map((tarefa) => ({
    ...
    detailHref: `/modulos/gkli-new/tarefas/${tarefa.id}`,
...
export async function getGkitNewDashboardData(): Promise<GkitNewDashboardData> {
  ...
  const readiness: GkitNewListRow[] = [
    ...
    clientes.length ? null : {
      ...
      meta: 'Sprint 1',
      detailHref: '/modulos/gkli-new/clientes/novo',
      tone: 'warning',
    },
    contatos.length ? null : {
      ...
      value: 'Base cadastral',
      meta: 'Sprint 1',
      detailHref: '/modulos/gkli-new/contatos/novo',
      tone: 'warning',
    },
    modelos.length ? null : {
      id: 'workflow',
      ...
      value: 'Workflow',
      meta: 'Sprint 1',
      detailHref: '/modulos/gkli-new/base/workflow/novo',
      tone: 'warning',
    },
